...
import { videoAnalysisApi, asyncVideoAnalysisApi, VideoAnalysisResult } from '@/lib/api';
import { UploadIcon, Loader2, Clock, CheckCircle, FileVideo, AlertTriangle, Info } from 'lucide-react';
...
export function VideoUpload({ onAnalysisComplete }: VideoUploadProps) {
  ...
  const [asyncStatusCheckInterval, setAsyncStatusCheckInterval] = useState<NodeJS.Timeout | null>(null);
  ...
  const checkAsyncStatus = useCallback(async (id: string) => {
    try {
    ...
    try {
      ...
      if (checkProfanity && !checkNsfw && !checkViolence) {
        ...
        if (!result.detailed_results && result.timestamp_results) {
          result.detailed_results = result.timestamp_results.map((item: any) => ({
  ...
  const handleAsyncAnalysis = async () => {
    ...
    try {
      ...
      const uploadResponse = await asyncVideoAnalysisApi.uploadVideo(file, contentId, true);
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="content-id" className="text-base font-medium">Content ID</Label>
              <Input
                id="content-id"
                value={contentId}
              ...
              >
                ...
                {file ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      <p className="text-base font-medium">Drag & drop a video file here</p>
                      <p className="text-sm text-muted-foreground">
            ...
            <Tabs defaultValue="sync" onValueChange={(value) => setAnalysisMode(value as 'sync' | 'async')} className="mt-6">
              <TabsList className="grid w-full grid-cols-2 p-1">
                <TabsTrigger value="sync" className="rounded-md py-2">
              ...
              <TabsContent value="sync" className="space-y-6 pt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label htmlFor="frame-interval" className="text-sm font-medium">Frame Interval</Label>
                    <Input
                      id="frame-interval"
                      type="number"
                      min="1"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className="flex items-center space-x-2 bg-muted/30 p-3 rounded-md">
                      <Checkbox 
                        id="check-nsfw" 
                        checked={checkNsfw} 
                        onCheckedChange={(checked) => setCheckNsfw(checked === true)}
                        className="data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground"
                      />
                      <Label htmlFor="check-nsfw" className="cursor-pointer text-sm">NSFW Content</Label>
                    </div>
                    ...
                    <div className="flex items-center space-x-2 bg-muted/30 p-3 rounded-md">
                      <Checkbox 
                        ...
                        onCheckedChange={(checked) => setCheckViolence(checked === true)}
                        className="data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground"
                      />
                      <Label htmlFor="check-violence" className="cursor-pointer text-sm">Violence</Label>
                    </div>
                    ...
                    <div className="flex items-center space-x-2 bg-muted/30 p-3 rounded-md">
                      <Checkbox 
                        ...
                        onCheckedChange={(checked) => setCheckProfanity(checked === true)}
                        className="data-[state=checked]:bg-primary data-[state=checked]:text-primary-foreground"
                      />
                      <Label htmlFor="check-profanity" className="cursor-pointer text-sm">Profanity</Label>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 border border-blue-100 p-4 rounded-md flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-blue-700 font-medium">Synchronous Analysis</p>
                    <p className="text-xs text-blue-600 mt-1">
                      The video will be analyzed immediately and results will be returned when complete. Best for smaller videos (under 50MB).
                    </p>
                  </div>
                </div>
              </TabsContent>
              ...
              <TabsContent value="async" className="space-y-6 pt-4">
                <div className="bg-amber-50 border border-amber-100 p-4 rounded-md flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm text-amber-700 font-medium">Asynchronous Analysis</p>
                    <p className="text-xs text-amber-600 mt-1">
                      The video will be uploaded and processed in the background. This is recommended for larger videos. You'll be redirected to the results page when the analysis is complete.
                    </p>
                  </div>
                </div>
                ...
                {asyncStatus && (
                  <div className={`p-4 rounded-md ${
                    asyncStatus === 'completed' ? 'bg-green-50 border border-green-100 text-green-700' :
                    asyncStatus === 'failed' ? 'bg-red-50 border border-red-100 text-red-700' :
                    'bg-blue-50 border border-blue-100 text-blue-700'
                  }`}>
                    <div className="flex items-center gap-2">
                      {asyncStatus === 'uploading' && <UploadIcon className="h-5 w-5 animate-pulse" />}
                      {asyncStatus === 'processing' && <Clock className="h-5 w-5 animate-pulse" />}
                      {asyncStatus === 'completed' && <CheckCircle className="h-5 w-5" />}
                      <p className="text-sm font-medium capitalize">{asyncStatus}</p>
                    </div>
                  </div>
                )}
              </TabsContent>
            ...
            >
              {isUploading ? (
                <>
                  ...
                  {analysisMode === 'sync' ? 'Analyzing...' : 
                   asyncStatus === 'uploading' ? 'Uploading...' : 'Processing...'}
                </>
              ) : (
                analysisMode === 'sync' ? 'Analyze Video' : 'Upload & Analyze'
              )}
            </Button>
          </CardFooter>
        </form>
