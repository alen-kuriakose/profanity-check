...
def get_kafka_producer():
    ...
    if _producer is None and _kafka_available:
        try:
            ...
            logger.info(
                f"Kafka producer initialized with bootstrap servers: {kafka_config.bootstrap_servers}")
        except NoBrokersAvailable as e:
            logger.warning(
                f"No Kafka brokers available: {str(e)}. Using direct processing instead.")
...
def send_message(topic: str, message: Dict[str, Any], key: str = None):
    ...
    if not _kafka_available:
        logger.info(
            f"Kafka not available, processing message directly: {message}")

        if topic == TOPIC_VIDEO_UPLOADED:
            try:

                coordinator_module = importlib.import_module(
                    'modules.video_analysis.workers.coordinator')
    ...
    try:
        ...
        if producer is None:
            logger.warning(
                "Kafka producer is None, processing message directly")
            if topic == TOPIC_VIDEO_UPLOADED:
        ...
        logger.info(
            f"Message sent to topic {topic} [{record_metadata.partition}] at offset {record_metadata.offset}")
        return True
    except NoBrokersAvailable:
        logger.warning(
            "No Kafka brokers available, switching to direct processing")
        _kafka_available = False
        return send_message(topic, message, key)
    except Exception as e:
...
def send_video_uploaded_message(video_id: int, content_id: str, file_path: str):
    ...
    logger.info(
        f"Preparing to send video uploaded message for content_id: {content_id}, video_id: {video_id}")
    message = {
        "video_id": video_id,
    ...
    logger.info(
        f"Video uploaded message for content_id: {content_id} sent: {result}")
    return result
