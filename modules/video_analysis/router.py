...
from modules.video_analysis.nsfw_checker import analyse_smol_vlm, detect_nsfw_falconsai
...
try:
    ...
    logger.warning(
        "Hugging Face content detection functionality may be limited")
...
async def analyze_video(
    ...
    check_profanity: bool = Form(
        True, description="Check for profanity in audio"),
    model_size: str = Form(
        "tiny", description="Whisper model size for audio transcription"),
    language: Optional[str] = Form(
        None, description="Language code for audio transcription"),
    confidence_threshold: float = Form(
        0.5, description="Minimum confidence threshold for detection"),
    use_smolvlm: bool = Form(
        True, description="Use SmolVLM for NSFW content detection")
):
    """
    ...
    logger.info(
        f"Processing video with content type: {file.content_type}, filename: {file.filename}")
    ...
    try:
        ...
        logger.info(
            f"Processing video for comprehensive analysis: {file.filename}")

        # Process frames
        frame_results = []
        inappropriate_frames = 0
        processed_frames = 0
        model_responses = []  # Initialize model_responses here
        ...
        # Use SmolVLM for NSFW detection if requested
        print("Logging check_nsfw and use_smolvlm",check_nsfw, use_smolvlm)
        if check_nsfw and use_smolvlm:
            logger.info("Using SmolVLM for NSFW content detection")
            from .nsfw_checker import analyse_video
            
            # Process the video frames using SmolVLM
            nsfw_result = analyse_video(tmp_path, frames_to_sample_per_minute=frame_interval)
            
            # Extract NSFW information
            nsfw_frames = len(nsfw_result.get("potential_profanity_incidents", []))
            processed_frames = nsfw_result.get("frames_analyzed", 0)
            max_nsfw_confidence = 0.9 if nsfw_frames > 0 else 0.0
            
            # Store model responses immediately
            model_responses = nsfw_result.get("model_responses", [])
            
            # Add model responses to frame results
            for response in nsfw_result.get("model_responses", []):
                frame_num = response.get("frame_number", 0)
                is_inappropriate = response.get("parsed_json", {}).get("inappropriate") == "YES"
                
                frame_result = {
                    "frame_number": frame_num,
                    "timestamp_seconds": (frame_num / (30/frame_interval)) * 60,
                    "timestamp_formatted": format_timestamp((frame_num / (30/frame_interval)) * 60),
                    "has_inappropriate_content": is_inappropriate,
                    "nsfw": {
                        "detected": is_inappropriate,
                        "confidence": 0.9 if is_inappropriate else 0.1,
            ...
            # Continue with other checks (violence, profanity)
        else:
            # Original frame-by-frame processing
        ...
        if check_profanity:
            try:
                ...
                logger.info(
                    f"Checking audio for profanity using Whisper model: {model_size}")
                ...
                audio_has_profanity = audio_profanity_results.get(
                    "has_profanity", False)
                audio_profanity_confidence = audio_profanity_results.get(
                    "confidence", 0.0)
                audio_profanity_segments = audio_profanity_results.get(
                    "timestamp_results", [])
                audio_transcript = audio_profanity_results.get(
                    "transcript", [])
                ...
                if audio_has_profanity:
                    ...
                    logger.info(
                        f"Audio profanity detected with confidence {audio_profanity_confidence}")
                    logger.info(
                        f"Found {len(audio_profanity_segments)} segments with profanity")
            except Exception as e:
        ...
        summary = {
            ...
            "profanity": {
                ...
                "full_transcript_available": audio_profanity_results is not None and "all_segments" in audio_profanity_results
            },
        ...
        logger.info(
            f"Completed comprehensive analysis: {processed_frames} frames, {inappropriate_frames} with inappropriate content")
        ...
        for result in frame_results:
            if result["nsfw"]["detected"]:
                flags.append({
                    ...
                    "model_response": result["nsfw"].get("model_response", "")
                })
            ...
            if not existing_video:
                ...
                db.update_combined_analysis(
                    ...
                    inappropriate_percentage=(
                        inappropriate_frames / processed_frames * 100) if processed_frames > 0 else 0,
                    result_data=result_data_json
                )

                logger.info(
                    f"Stored synchronous analysis results in database for content_id: {content_id}")
            else:
                logger.info(
                    f"Video with content_id {content_id} already exists in database, skipping storage")
        except Exception as e:
            logger.error(
                f"Failed to store synchronous analysis results in database: {e}")
        ...
        response = {
            ...
            "detailed_results": frame_results,
            "model_responses": model_responses,
            "model_info": "Using SmolVLM for NSFW detection" if (check_nsfw and use_smolvlm) else "Using Hugging Face models for NSFW and violence detection"
        }
        ...
        if audio_profanity_results and "transcript" in audio_profanity_results:
            response["transcript"] = {
                ...
                "segments_with_profanity": audio_profanity_results.get("segments_with_profanity", []),
                "all_segments": audio_profanity_results.get("all_segments", [])
            }
...
async def video_profanity_check(
    file: UploadFile = File(...),
    content_id: str = Form(
        None, description="Optional content ID for database storage"),
    model_size: str = Form(
        "tiny", description="Whisper model size (tiny, base, small, medium, large)"),
    language: str = Form(
        None, description="Optional language code for transcription")
):
    """
            ...
            try:
                ...
                if not existing_video:
                    ...
                    profanity_id = db.insert_profanity_analysis(
                        ...
                        profanity_frames=len(
                            timestamp_results) if has_profanity else 0,
                        frames_analyzed=1,
                        max_profanity_confidence=result["confidence"] if has_profanity else 0.0,
                        transcript=transcript,
                        all_segments=json.dumps(
                            result.get("all_segments", [])),
                        segments_with_profanity=json.dumps(
                            result.get("segments_with_profanity", [])),
                        full_transcript_available=("all_segments" in result),
                        result_data={
                    ...
                    logger.info(
                        f"Stored profanity analysis results in database for content_id: {content_id}")
                else:
                    logger.info(
                        f"Video with content_id {content_id} already exists in database, skipping storage")
            except Exception as e:
                logger.error(
                    f"Failed to store profanity analysis results in database: {e}")
                # Continue even if database storage fails
    ...
    # Add transcript data in a structured format for the frontend
...
async def check_profanity_in_video(
    ...
    frame_interval: int = Query(
        15, ge=1, description="Process every Nth frame (lower = more frames processed)"),
    enhance_frames: bool = Query(
        True, description="Enhance frames for better OCR"),
    auto_optimize: bool = Query(
        True, description="Auto-optimize settings based on video quality"),
    resize_max_dimension: int = Query(
        1280, ge=100, description="Resize larger frames to this max dimension"),
    min_text_confidence: float = Query(
        20.0, ge=0, le=100, description="Minimum OCR confidence for a word to be included"),
    apply_threshold: bool = Query(
        True, description="Apply adaptive thresholding to improve text contrast"),
    apply_denoise: bool = Query(
        True, description="Apply denoising filter (slower but may improve results)"),
    ocr_config: str = Query('--oem 3 --psm 11 -l eng --dpi 300',
                            description="Tesseract OCR configuration string")
    ...
    try:
        ...
        for frame_num, timestamp, frame in extract_frames(tmp_path, frame_interval=frame_interval):
            ...
            result = find_profanity_with_paddle(
                ...
                min_confidence=min_text_confidence/100.0
            )

            # If PaddleOCR didn't find any text, fall back to Tesseract
            if not result.get('text'):
                logger.info(
                    f"PaddleOCR found no text in frame {frame_num}, trying Tesseract...")
        ...
        summary["text_detection_rate"] = (
            frames_with_text / processed_frames * 100) if processed_frames else 0

        # Add OCR engine info
        summary["ocr_engines"] = [
            "PaddleOCR (primary)", "Tesseract OCR (fallback)"]
        summary["ocr_config"] = {
        ...
        logger.info(
            f"Completed analysis: {processed_frames} frames, {num_profanity} with profanity, {frames_with_text} with text detected")
...
async def check_violence_in_video(
    video: UploadFile = File(...),
    frame_interval: int = Query(
        30, ge=1, description="Process every Nth frame"),
    confidence_threshold: float = Query(
        0.5, ge=0.0, le=1.0, description="Minimum confidence threshold for violence detection"),
    resize_max_dimension: int = Query(
        480, ge=100, description="Resize larger frames to this max dimension for faster processing")
) -> JSONResponse:
    """
    ...
    if use_mock_implementation:
        logger.warning(
            "Using mock violence detection implementation as the Hugging Face models are not available")
    ...
    try:
        ...
        for frame_num, timestamp, frame in extract_frames(tmp_path, frame_interval=frame_interval):
            ...
            try:
                ...
                if use_mock_implementation:
                    ...
                    frame_prediction = {k: v/total for k,
                                        v in frame_prediction.items()}
                    logger.debug(
                        f"Using mock predictions for frame {frame_num}")
                else:
                ...
                max_violence_confidence = max(
                    max_violence_confidence, violence_confidence)
        ...
        logger.info(
            f"Completed violence analysis: {processed_frames} frames, {violent_frames} with violent content")
...
async def check_nsfw_in_video(
    ...
    frame_interval: int = Query(
        30, ge=1, description="Process every Nth frame"),
    confidence_threshold: float = Query(
        0.5, ge=0.0, le=1.0, description="Minimum confidence threshold for NSFW detection"),
    resize_max_dimension: int = Query(
        480, ge=100, description="Resize larger frames to this max dimension for faster processing")
) -> JSONResponse:
    """
    ...
    try:
        ...
        logger.info(f"Processing video for NSFW content analysis: {video.filename}")

        # Use the SmolVLM analysis function
        from .nsfw_checker import analyse_video
        
        # Process the video frames using SmolVLM
        result = analyse_video(tmp_path, frames_to_sample_per_minute=30/frame_interval)
        
        # Format the result to match the expected structure
        formatted_result = {
            "status": "completed",
            "content_rating": "explicit" if result.get("potential_profanity_incidents") else "safe",
            "summary": {
                "total_frames_analyzed": result.get("frames_analyzed", 0),
                "frames_with_nsfw_content": len(result.get("potential_profanity_incidents", [])),
                "nsfw_percentage": (len(result.get("potential_profanity_incidents", [])) / result.get("frames_analyzed", 1) * 100) if result.get("frames_analyzed", 0) > 0 else 0,
                "max_nsfw_confidence": 0.9 if result.get("potential_profanity_incidents") else 0.0
            },
            "flags": [
                {
                    "type": "explicit",
                    "confidence": 0.9,
                    "timestamp": incident.get("timestamp_ms", 0) / 1000,
                    "timestamp_formatted": incident.get("timestamp_formatted", "00:00.000"),
                    "frame_number": incident.get("frame_number", 0),
                    "model_response": incident.get("model_response", "")
                } for incident in result.get("potential_profanity_incidents", [])
            ],
            "detailed_results": result.get("model_responses", []),
        ...
        # Store results in database if content_id is provided
        if content_id:
            try:
                ...
                if not existing_video:
                    # Save video metadata to database
                    with NamedTemporaryFile(delete=False, suffix=video.filename) as saved_file:
                        video.file.seek(0)  # Reset file pointer
                        shutil.copyfileobj(video.file, saved_file)
                        saved_path = saved_file.name
                    ...
                    db.update_combined_analysis(
                        ...
                        status="completed",
                        content_rating=result["content_rating"],
                        inappropriate_frames=result["summary"]["frames_with_nsfw_content"],
                        total_frames_analyzed=result["summary"]["total_frames_analyzed"],
                        inappropriate_percentage=result["summary"]["nsfw_percentage"],
                        result_data=result_data_json
                    )

                    logger.info(f"Stored NSFW analysis results in database for content_id: {content_id}")
                else:
                    logger.info(f"Video with content_id {content_id} already exists in database, skipping storage")
            except Exception as e:
                logger.error(f"Failed to store NSFW analysis results in database: {e}")
                # Continue even if database storage fails
